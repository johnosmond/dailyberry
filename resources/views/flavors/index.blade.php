            ...
            <form name="frmSearch" id="frmSearch" method="GET" action="{{ route('flavors.index') }}">
                <div class="flex gap-2 md:gap-4">
                    ...
                    <div class="md:w-1/2 text-right pr-2 md:pr-10">
                        ...
                        <select name="display_count" id="display_count" class="select"
                        ...
                        <thead>
                            <tr class="text-gray-500">
                                <th class="w-[30%] px-4 py-2 md:w-[20%]"><a
                                        href="{{ route('flavors.index') . '?sort=flavor' . $dirstr }}" class="text-base md:text-lg font-bold">Flavor</a></th>
                                <th class="w-[70%] px-4 py-2"><span class="text-base md:text-lg font-bold">Description</span></th>
                                <th class="w-[10%] px-4 py-2 hidden md:table-cell"><a
                                        href="{{ route('flavors.index') . '?sort=times_used' . $dirstr }}" class="text-base md:text-lg font-bold"># <span
                        ...
                        <tbody>
                            @foreach ($flavors as $flavor)
                                <tr class="odd:bg-gray-100">
                                    <td class="text-sm md:text-base border px-4 py-2">{{ $flavor->flavor }}</td>
                                    <td class="text-sm md:text-base border px-4 py-2">{{ $flavor->description }}</td>
                                    <td class="text-sm md:text-base border px-4 py-2 hidden md:table-cell">{{ $flavor->times_used }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
